...
export const DarkModeToggle = ({ mode, setMode }) => {
  return (
    <button
      onClick={() => setMode(mode === "dark" ? "light" : "dark")}
      className={`p-2 w-20 md:w-28 rounded-full flex shadow-lg relative bg-gradient-to-b ${
        mode === "light"
          ? "justify-end from-lavenda to-rose"
          : "justify-start from-jacarta to-plum"
      }`}
    >
      <Thumb mode={mode} />
      {mode === "light" && <Clouds />}
...
const Thumb = ({ mode }) => {
  return (
    <motion.div
      ...
      className="h-8 w-8 md:h-10 md:w-10 rounded-full overflow-hidden shadow-lg relative"
    >
      <div
...
const SunCenter = () => (
  <div className="absolute inset-1 md:inset-1.5 rounded-full bg-amber-300" />
);
